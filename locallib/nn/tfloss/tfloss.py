...
class TfLosses:
    ...
    @staticmethod
    def softmax_loss(embedding, labels, out_num, act, reg, init):
        #eps=1e-10
        #logitsn = tf.clip_by_value(logits, eps, 1.0-eps)
        logits = dense(inputs=embedding, units=out_num, activation=act, kernel_regularizer=reg, kernel_initializer=init)
        softmax_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=logits))    
        return logits, softmax_loss

    @staticmethod
    def arc_loss(embedding, labels, out_num, w_init=tf.contrib.layers.xavier_initializer(uniform=False), s=64., m=0.5):
        '''
        ...
        embedding_norm = tf.math.l2_normalize(embedding, axis=1, name='norm_embedding')

        
        weights = tf.get_variable(name='embedding_weights', shape=(embedding.get_shape().as_list()[-1], out_num),
                                  initializer=w_init, dtype=tf.float32, trainable=True)
        weights_norm = tf.math.l2_normalize(weights, axis=0, name='norm_weights')
        
        # cos(theta+m)
        cos_t = tf.matmul(embedding_norm, weights_norm, name='cos_t')
        ...
        return logit, inference_loss, 


    @staticmethod
    def cosine_loss(embedding, labels, out_num, w_init=None, s=30., m=0.4, scope='cosine_loss'):
        '''
        :param embedding: the input embedding vectors
        :param labels:  the input labels, the shape should be eg: (batch_size, 1)
